...
`5240_4` の初任務後の門派説明を場面単位で監査する。

重点は、宇文逸の `兄さん` 誤訳、莫問の `あれを訪ねる` という指示対象崩れ、翻訳調の感謝、帰室時の `師兄また`。敬語を消すのではなく、弟弟子の素直さと兄弟子の説明の自然な発話へ戻す。既存修正束に収録済みのキーは新束へ重複させず、元の束を直接再改訂する。
