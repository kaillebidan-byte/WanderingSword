...
- `private_preparation`: 複数candidate packetを先に準備し、queueをsealする。fix / keep、fix JSON、owner新設、正式束番号は禁止。
- `private_quality_audit`: sealed queue全体を連続監査する。件数、release残量、metricsを見せない。一packetごとにencodingへ移らない。
- `private_encoding`: 全監査済みpacketをまとめて収録する。新しい翻訳判断は禁止。疑義packetだけを`needs_reaudit`へ戻す。
...
輸送は翻訳段階を`translation_frozen`に保ったまま進める。
...
- PR #118はsquash統合済み。
- merge SHAは`56c8eecfc4eb1a44a23708d25d9566d68cea016c`。
- verified checkpointは第80束、人物ペア1171、全体1529。
- train-06 releaseは`yuwen-mowen-train-06-r1`。
- train-06の翻訳段階は`translation_frozen`、輸送は`merged`。
- `5581_7 + 5581_8`は予約だけで、preparation・quality audit・encodingは未開始。
- 四段階wave v2の制度改修PRを翻訳列車PRから分離する。
...
- 制度改修PRへ訳文、fix JSON、人物owner、FACT_DOUBT、ALLUSION_REVIEWを混ぜること
