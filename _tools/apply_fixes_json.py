# -*- coding: utf-8 -*-
"""汎用: {"<tbl>\\x1f<ns>\\x1f<key>": "新・値全体", ...} のJSONを locres に書込＋_work repak。
cursorは進めない・deployしない。使い方: python apply_fixes_json.py fixes.json [--apply]
"""
import os, sys, glob, struct, subprocess, json
...
def main():
    ...
    by_t = {}
    for full, nv in fixes.items():
        tbl, ns, key = full.split("\x1f", 2)
    ...
    for tbl, items in by_t.items():
        ...
        for ns, key, nv in items:
            ...
            if i is None:
                print(f"  ⚠ key無し {tbl}|{key}"); continue
            if arr[i][0] != nv:
                arr[i][0] = nv; n += 1
        if apply and n:
            open(lp, "wb").write(b[:arr_off] + L.write_string_array(arr, ver))
        print(f"  {tbl}: {n}件" + ("書込" if apply else "(変更予定)"))
        total += n
    if not apply:
        print(f"[プレビュー] 計{total}件 / --apply で書込"); return
    ...
    print(f"✅ 計{total}件書込 / repak -> {outpak}  (deployは未実施)")
