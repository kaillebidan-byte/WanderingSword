...
出力: /tmp/pf_batch.json に [{target,ns,key,zh,ja}] を書き、標準出力にも表示。
...
def main():
    ...
    if not batch:
        print("校正キュー全完了");
        json.dump([], open("/tmp/pf_batch.json", "w"))
        return
    # 必要なtargetのJAだけ読む
    ja_cache = {}
    out = []
    ...
    json.dump(out, open("/tmp/pf_batch.json", "w", encoding="utf-8"), ensure_ascii=False, indent=1)
