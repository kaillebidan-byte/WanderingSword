#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""CI_TRAIN_MANIFEST.jsonの正式束、輸送集計、checkpoint整合を検査する。"""
...
def _compare_mapping(label: str, observed: dict[str, Any], expected: dict[str, Any], errors: list[str]) -> None:
    for key, value in expected.items():
        if observed.get(key) != value:
            errors.append(f"{label} {key} mismatch: observed={observed.get(key)!r} expected={value!r}")
...
def validate_manifest(
    ...
    *,
    ...
    if manifest.get("phase") != "phase1_wave":
        errors.append("phase must be phase1_wave")
    if "candidate_packets" in manifest:
        errors.append("candidate_packets must be stored in PRIVATE_STAGE_STATE")
    ...
    if manifest.get("thresholds") != PILOT_THRESHOLDS:
        errors.append(f"thresholds must equal phase1 wave values {PILOT_THRESHOLDS!r}")
    if manifest.get("caps") != PILOT_CAPS:
        errors.append(f"caps must equal phase1 wave values {PILOT_CAPS!r}")
    ...
    for key in ("phase", "train_id", "branch", "status"):
        if current_train.get(key) != manifest.get(key):
            errors.append(f"CURRENT_WORK.ci_train.{key} mismatch: current={current_train.get(key)!r} manifest={manifest.get(key)!r}")
    ...
    if not isinstance(transport, dict):
        errors.append("manifest.transport must be an object")
    elif current_train.get("transport_status") != transport.get("status"):
        errors.append("CURRENT_WORK.ci_train.transport_status mismatch")
    ...
    if base_batch != current_train.get("base_checkpoint_batch"):
        errors.append("base_checkpoint batch must match CURRENT_WORK.ci_train.base_checkpoint_batch")
    checkpoint_batch = checkpoint.get("batch")
    checkpoint_advanced = isinstance(base_batch, int) and isinstance(checkpoint_batch, int) and checkpoint_batch > base_batch
    if not checkpoint_advanced:
        ...
        if checkpoint.get("status") not in {"pending_audit_sync", "verified"}:
            errors.append("advanced train checkpoint must be pending_audit_sync or verified")
    ...
    sums = {"bundle_count": len(bundles), "reviewed_rows": 0, "fix_keys": 0, "new_pair_keys": 0}
    ...
    for index, bundle in enumerate(bundles):
        ...
        if not isinstance(bundle, dict):
            ...
            continue
        if "status" in bundle:
            errors.append(f"{label}.status is deprecated; use review_status and apply_status")
        if bundle.get("review_status") != "complete":
            errors.append(f"{label}.review_status must be complete")
        apply_status = bundle.get("apply_status")
        if apply_status not in {"pending", "verified"}:
            errors.append(f"{label}.apply_status must be pending or verified")
        if status == "verified" and apply_status != "verified":
            errors.append(f"{label}.apply_status must be verified when manifest is verified")
        ...
        if not _positive_int(batch):
            ...
            if expected_batch is not None and batch != expected_batch:
                errors.append(f"bundle batches must be consecutive from checkpoint: expected {expected_batch}, got {batch}")
        ...
        if not isinstance(review, str) or not review.startswith("_phase4_proofread/REVIEW_") or not review.endswith(".md"):
    ...
    if checkpoint_advanced:
        ...
        if checkpoint_batch != last_bundle_batch:
            errors.append("advanced checkpoint batch must equal final train bundle")
    ...
    for key, expected in sums.items():
        if totals.get(key) != expected:
            errors.append(f"totals.{key} mismatch: manifest={totals.get(key)!r}, calculated={expected}")
    ...
    if trigger is not None:
        if not isinstance(trigger, dict):
            ...
            if trigger.get("reason") not in ALLOWED_EARLY_RELEASE:
                errors.append(f"release_trigger.reason is not allowed: {trigger.get('reason')!r}")
            if not isinstance(trigger.get("detail"), str) or not trigger.get("detail", "").strip():
    ...
    if status == "accumulating" and ready:
        errors.append("manifest reached a release condition but status is still accumulating: " + ", ".join(reasons))
    ...
    if status in {"ready_for_public_ci", "in_public_ci"} and declared != "translation_frozen":
        errors.append("ready train requires operation_mode translation_frozen")
    return errors
...
def main() -> int:
    ...
    print(f"totals: {totals.get('bundle_count')} bundle(s), {totals.get('reviewed_rows')} row(s), {totals.get('fix_keys')} fix key(s)")
    ...
    if errors:
        ...
        return 1
    print("OK: CI train manifest contains encoded formal bundles only")
    return 0
